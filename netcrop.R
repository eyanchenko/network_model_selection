...
netcrop_sbm <- function(A, K.CAND,
  ...
  obj <- data.table(`Candidate Model` = rep(mod, length(K.CAND)),
                    `Candidate Value` = rep(K.CAND, each = 1)
  )
  ...
  for(lq in seq_along(loss))
    for(r in 1:R){
      obj[[paste0(loss[lq], "-Rep=", r)]] <- 
        as(rbind(L[[r]][paste0("SBM_",loss[lq]), ]), "vector")
    }
  
  obj2 <- list()
  
  obj2[["Candidate Models"]] <- "SBM"
...
croissant.rdpg <- function(A, d.cand, s, o, R,
  ...
  raw.out <- mclapply(1:nrow(raw.ind), function(ii){
    ...
    U <- eig$v
    sigma.half <- diag(sqrt(abs(eig$d))) # I think this will give an error when dmax = 1 (as the matrix will be ones on diagonal)
...
##ECV for RDPG
missing.undirected.Rank.weighted.fast.all <- function(holdout.index, A, D.CAND, soft=FALSE,fast=fast,p.sample=1){
  n <- nrow(A)
  #A.new <- A
  ...
  Omega <- which(is.na(A.new))
  imputed.A <- list()
  sse <- rep(0,length(D.CAND))
  SVD.result <- iter.SVD.core.fast.all(A.new, max(D.CAND),fast=TRUE,p.sample=p.sample)
  for(ii in 1:length(D.CAND)){
    d = D.CAND[ii]
    #print(d)
    tmp.est <- SVD.result[[d]]
    #if(k==1){
    ...
    #aa <- AUC::roc(predictions=predictors,labels=factor(response))
    sse[ii] <- mean((response-predictors)^2)
    imputed.A[[ii]] <- A.approx
  }
...
ecv_rdpg <- function(A,D.CAND,B=3,holdout.p=0.1,soft=FALSE,fast=fast){
  n <- nrow(A)
  #edge.index <- 1:n^2
  #edge.n <- length(edge.index)
  edge.index <- which(upper.tri(A))
  ...
  result <- lapply(holdout.index.list,
                   missing.undirected.Rank.weighted.fast.all,
                   A=A,D.CAND=D.CAND,soft=soft,fast=fast,p.sample=1-holdout.p)
  
  sse.mat <- matrix(0,nrow=B,ncol=length(D.CAND))
  ...
  return(list(rank.sse=D.CAND[which.min(sse.seq)], sse=sse.seq
  ))
}
